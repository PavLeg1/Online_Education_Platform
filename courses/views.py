...
# Получает объект Module с заданным id, который принадлежит текущему 
# пользователю и прорисовывает шаблон в данным модулем  
...
# Извлечение и прорисовка курсов по предмету
class CourseListView(TemplateResponseMixin, View):
    model = Course
    template_name = 'courses/course/list.html'

    def get(self, request, subject=None):
        # Пытаемся получить клбч из all_subjects кеша
        # Если нет ключа - None
        # Если не найден, то выполняем запрос на извлечение 
        # всех Subject и их число курсов, результат кешируется  
        subjects = cache.get('all_subjects')
        if not subjects:
            # извлекаются все предметы, чтобы вставить число курсов по каждому
        ...
        # Если предмет есть - ключ создается динамически
        if subject:
        ...
        # Если ключа нет - все курсы кэшируются ключом all_courses
        else:
...
class CourseDetailView(DetailView):
    ...
    # Вставка формы зачисления в контекст прорисовки шаблонов
